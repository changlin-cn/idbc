...
export default () =>{
    ...
    const handleDelete:React.MouseEventHandler<HTMLButtonElement> = useCallback(
        (event)=>{
            ...
            if(id){
                ...
                IDBDemo.clear('IDBDemoStore').then(
                    ()=>  IDBDemo.getAll('IDBDemoStore').then(setList)
                    );
            }
    ...
    const handleUpdate:React.MouseEventHandler<HTMLButtonElement> = useCallback(
        (event)=>{
            const id =  event.target.getAttribute('data-id');
    ...
    return <div>
        ...
        <ul>
            {list.length>0 && list.map(n=>
                (<li key={n.recordId}>
                    <button data-id={n.recordId} onClick={handleUpdate}>update</button>&nbsp; 
                    <button data-id={n.recordId} onClick={handleDelete}>remove</button>&nbsp; 
                    recordId:<b>{n.recordId}</b>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  value:<b>{n.value}</b> 
                    </li>)
                )}
            <li>
